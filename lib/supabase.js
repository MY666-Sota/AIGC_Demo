...
export const db = {
  ...
  async searchKnowledgeBase(query, limit = 10) {
    ...
    const uniqueTerms = [...new Set(rawTerms)].slice(0, 12);

    const scoredRows = (data || []).map((row) => {
      const styleName = (row.style_name || '').toLowerCase();
      const category = (row.category || '').toLowerCase();
      const promptTemplate = (row.prompt_template || '').toLowerCase();
      const metadataText = JSON.stringify(row.metadata || {}).toLowerCase();
      const haystack = `${styleName} ${category} ${promptTemplate} ${metadataText}`;

      let score = 0;

      if (styleName && normalizedQuery.includes(styleName)) score += 12;
      if (category && normalizedQuery.includes(category)) score += 8;

      for (const term of uniqueTerms) {
        if (styleName.includes(term)) score += 6;
        if (category.includes(term)) score += 4;
        if (promptTemplate.includes(term)) score += 3;
        if (metadataText.includes(term)) score += 2;
      }

      if (score === 0 && haystack.includes(normalizedQuery)) {
        score += 5;
      }
